...
class PenBlocks {
	...
	_clampPenSize (requestedSize) {
		return MathUtil.clamp(
			requestedSize,
			PenBlocks.PEN_SIZE_RANGE.min,
			PenBlocks.PEN_SIZE_RANGE.max
		);
	}
	 ...
	 */
	_getPenState (target) {
		let penState = target.getCustomState(PenBlocks.STATE_KEY);
		if (!penState) {
			penState = Clone.simple(PenBlocks.DEFAULT_PEN_STATE);
			target.setCustomState(PenBlocks.STATE_KEY, penState);
		}
		return penState;
	}
	...
	_onTargetCreated (newTarget, sourceTarget) {
		if (sourceTarget) {
			const penState = sourceTarget.getCustomState(PenBlocks.STATE_KEY);
			if (penState) {
				newTarget.setCustomState(PenBlocks.STATE_KEY, Clone.simple(penState));
				if (penState.penDown) {
