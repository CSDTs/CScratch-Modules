////////////////////////////////////////////////////////////////////////////////////
/***********************************************************************************
all functions needed to create beadloom *********************************************
************************************************************************************/
/////////////////////////////////////////////////////////////////////////////////////
const NEW_BLOCK_STRING_iso = '<block type="pen_iso" id="pen_iso"><value name="beads"><shadow type="math_number"><field name="NUM">2</field></shadow></value>'
+'<value name="rows"><shadow type="math_number"><field name="NUM">3</field></shadow></value>'
+'<value name="total"><shadow type="math_number"><field name="NUM">20</field></shadow></value>'
+'<value name="direction"><shadow type="text"><field name="TEXT">-y</field></shadow></value></block>';
const NEW_BLOCK_CATEGORY_iso = 'Pen';
//
...
const NEW_BLOCK_CATEGORY_lineofbeads = 'Pen';
...
//
const NEW_BLOCK_STRING_gotobead = '<block type="motion_gotobead" id="motion_GOTOBEAD"><value name="NUM_X"><shadow type="math_number"><field name="NUM">0</field></shadow></value>'
+'<value name="NUM_Y"><shadow type="math_number"><field name="NUM">0</field></shadow></value></block>';
const NEW_BLOCK_CATEGORY_gotobead = 'Motion';
// go to bead 
vm.runtime._primitives.motion_gotobead = function (args, util) {
		let x = Number(args.NUM_X);
		let y = Number(args.NUM_Y);
		const size_x= (util.target.getBounds().right) - (util.target.getBounds().left);
		const size_y= (util.target.getBounds().top) - (util.target.getBounds().bottom);
		let dx=size_x;
		let dy=size_y;
		args.X= dx*x;
		args.Y= dy*y;
...
////////////////////////////////////////////////////////////////////////////////
/*** 
function for drawing iso triangle
***/
vm.runtime._primitives.pen_iso = function (args, util) {
		...
		for (j=0;j<total;j++){
			...
			args.NUMb= beadlength;
			args.rise=0;
			args.run= (-1-beadlength);
			vm.runtime._primitives.pen_lineofbeads(args, util);
			//
			args.STEPS= beadlength;
			vm.runtime._primitives.motion_translate_inx_nostamp(args, util);
			//
			iteration+=1;
			if (total != iteration){
...
ScratchBlocks.Blocks['pen_iso'] = {
  /**
   * draw isoceles triangle Block+.
   * @this Blockly.Block
   */
  init: function () {
    this.jsonInit ( {
        "message0":"iso triangle :add %1 beads every %2 rows for %3 iterations in direction %4",
		 args0:
...
************************************************************************************/
/////////////////////////////////////////////////////////////////////////////////////
let toolboxDOM_iso = (new
DOMParser).parseFromString(ScratchBlocks.Blocks.defaultToolbox, 'text/xml');
//
let newBlockDOM_rt = (new DOMParser).parseFromString(NEW_BLOCK_STRING_rt, 'text/xml');
let newBlockDOM_iso = (new DOMParser).parseFromString(NEW_BLOCK_STRING_iso,
'text/xml');
...
'text/xml');
//
toolboxDOM_iso.getElementsByName('Pen')[0].appendChild(newBlockDOM_iso.children[0]);
toolboxDOM_iso.getElementsByName('Motion')[0].appendChild(newBlockDOM_translatebyx.children[0]);
toolboxDOM_iso.getElementsByName('Motion')[0].appendChild(newBlockDOM_translatebyy.children[0]);
toolboxDOM_iso.getElementsByName('Pen')[0].appendChild(newBlockDOM_lineofbeads.children[0]);
toolboxDOM_iso.getElementsByName('Motion')[0].appendChild(newBlockDOM_translatebywidth.children[0]);
toolboxDOM_iso.getElementsByName('Motion')[0].appendChild(newBlockDOM_translatebyheight.children[0]);
toolboxDOM_iso.getElementsByName('Pen')[0].appendChild(newBlockDOM_rt.children[0]);
toolboxDOM_iso.getElementsByName('Motion')[0].appendChild(newBlockDOM_gotobead.children[0]);
//
let toolboxXML_iso = (new XMLSerializer).serializeToString(toolboxDOM_iso);
Blockly.getMainWorkspace().updateToolbox(toolboxXML_iso);